...
const Scanner: React.FC<ScannerProps> = ({ lang, onResult, onBack }) => {
  ...
  // SPLIT INTO TWO SEPARATE ERROR STATES
  ...
  const processImage = async (imageB64: string) => {
    ...
    try {
      ...
      // CASE 1 — Scan completely failed
      if (aiError === 'SCAN_FAILED' || !result) {
        setCameraError('Scan failed. Please try again.');
        return;
      }

      // CASE 2 — Plant not recognized (Strawberry, Apple, etc.)
      if (aiError === 'NOT_RECOGNIZED' || (result && result.isPlant === false)) {
        setNotRecognized(true); // show NOT RECOGNIZED modal
        return;
      }

      // CASE 3 — Valid plant (Tomato, Garlic, Red Onion)
      stopCamera();
  ...
  // Try again after not recognized — just reset, no camera restart
        ...
        {notRecognized && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 text-center">
              ...
              {/* Try Again — only resets state, no camera restart */}
              <button
